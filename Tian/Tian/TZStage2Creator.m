...
- (NSArray *)groupOfAnimationsToLayer:(CALayer *)layer
{
    ...
    NSArray *stringObjects = @[@"a toy",@"a person",@"a story"];
    NSArray *images = @[@"toy.jpg",@"person.jpg",@"story.jpg"];
    
    __block CALayer *lastImageLayer = nil;
    
    [stringObjects enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
        CGSize size = [self getSizeForText:obj forFont:font];
        
        TZAnimateOperation *bgOperation = [self animationForImage:[UIImage imageNamed:images[idx]] animatedLayer:layer];
        
        bgOperation.insertAtBack = YES;
        
        if (lastImageLayer) {
            bgOperation.layersToFade = @[lastImageLayer];
            bgOperation.fadeDuration = 0.66;
        }
        
        lastImageLayer = bgOperation.layer;
